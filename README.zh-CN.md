...
例如：`ansible-base==2.10.17 + Python 3.6` 会自动匹配 [locks/ansible-base-2.10.17-py36.txt](locks/ansible-base-2.10.17-py36.txt)。
...
这份人类可读的版本映射表维护在 [data/ansible_control_node_python.json](data/ansible_control_node_python.json)，构建和 `inspect-source` 会优先读取它；包自身的 `Requires-Python` 只作为技术性兜底。
...
默认会重写英文主文档 `README.md` 里的矩阵。如果你也要同步中文版，再额外执行一次：
...
当前实现会按 `--readme` 文件名自动选择矩阵语言：

- `README.md`：英文矩阵
- `README.zh-CN.md`：中文矩阵
